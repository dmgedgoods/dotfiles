...
base_prompt="%{$fg[white]%}%~% %(?.%{$fg[green]%}.%{$fg[red]%})%B❱%b "
custom_prompt=""
last_run_time=""
last_vcs_info=""
...
function pipestatus_parse {
  ...
  if [[ "$ERROR" -ne 0 ]]; then
      print "[%{$fg[red]%}$PIPESTATUS%{$fg[green]%}]"
  fi
}


# Combine it all into a final right-side prompt
PROMPT='%{$fg[green]%}[%~% ]%(?.%{$fg[green]%}.%{$fg[red]%})%B$%b '
function preexec() {
...
function precmd() {
    ...
    if [ ! -z "$last_run_time" ]; then
        ...
        info=$(printf "%s%s%s%s%s" "%{$fg[green]%}[" "$elapsed_color" "$elapsed" "%{$fg[green]%}]" "$RETVAL")
        unset last_run_time
    fi
...
function hg_prompt_info() {
    ...
    print "%{$fg[green]%}[${color}${branch}%{$fg[green]%}]"
}
